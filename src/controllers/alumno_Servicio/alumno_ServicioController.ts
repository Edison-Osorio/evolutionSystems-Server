...
class AlumnoSerController {
  ...
  public async getOneAlumno_Servicio(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const query = await pool.query(
        "SELECT * FROM alumno_servicio WHERE id_alumno_as = ? AND codigo_servicio_as = ? ",
  ...
  public async deleteAlumno_ServicioAlumno(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<any> {
    try {
      ...
      const query = await pool.query("DELETE FROM alumno_servicio WHERE id_alumno_as = ? ", [
        id_alumno,
      ]);
      res.json({ message: "Se ha eliminado el servicio del alumno" });
    } catch (error) {
  ...
  public async alumnoOutService(req: Request, res: Response, next: NextFunction) {
    try {
      const { id_alumno } = req.params;
      const query = await pool.query(
        "SELECT * FROM servicio WHERE servicio.id_servicio NOT IN (SELECT alumno_servicio.codigo_servicio_as FROM alumno_servicio WHERE alumno_servicio.id_alumno_as=?)",
        [id_alumno]
      );
      res.json(query);
    } catch (error) {
