...
class DocenteAsignaturaController {
    ...
    public async list(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const query = await pool.query('SELECT docente.nom_doc,nom_asi,desc_asi FROM asignatura INNER JOIN nota ON asignatura.id_asi=nota.id_asi INNER JOIN docente_asignatura ON asignatura.id_asi=docente_asignatura.id_asi INNER JOIN docente ON docente_asignatura.nif_doc=docente.nif_doc  WHERE docente_asignatura.nif_doc= ?', [nif_doc]);
            res.json(query);
        } catch (error) {
    ...
    //listar los grupos
    public async listGroup(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const { nif_doc, cod_gra } = req.params
            const query = await pool.query('SELECT grado.cod_gra,grado.nom_grad,grado.carac_grad FROM grado INNER JOIN docente_grado ON grado.cod_gra=docente_grado.cod_gra WHERE docente_grado.nif_doc= ? AND docente_grado.cod_gra= ?', [nif_doc, cod_gra]);
            res.json({ text: query });
            console.log(query);
